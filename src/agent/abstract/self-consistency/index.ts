...
export class SelfConsistency<Answer = any, Result = any> {
	...
	async invoke(options: InvokeOptions): Promise<SelfConsistencyResult<Answer, Result>> {
		return await withTelemetry("self_consistency.invoke", {
			"self_consistency.candidate_count": this.candidates.parallelRun.length,
			"self_consistency.min_agreement": this.minAgreement,
			"self_consistency.min_candidates": this.minCandidates
		}, async span => {
			...
			for (const [id, rawResult] of rawResults) {
				try {
					...
					span.addEvent("self_consistency.candidate", {
						id,
						weight,
					...
					span.addEvent("self_consistency.invalid_candidate", {
						id,
						error: invalidCandidate.error.message
					});
				}
			}
			...
			span.setAttribute("self_consistency.status", result.status);
			span.setAttribute("self_consistency.valid_candidates", result.candidates.length);
			span.setAttribute("self_consistency.invalid_candidates", result.invalidCandidates.length);
			span.setAttribute("self_consistency.cluster_count", result.clusters.length);
			span.setAttribute("self_consistency.agreement", result.agreement);
			if (result.winner) {
				span.setAttribute("self_consistency.winner", result.winner.normalizedAnswer);
			}
			if (result.status === "abstained") {
				span.setAttribute("self_consistency.abstention_reason", result.reason);
			}
			span.addEvent("self_consistency.decision", {
				status: result.status,
				agreement: result.agreement,
				valid_candidates: result.candidates.length,
				invalid_candidates: result.invalidCandidates.length,
