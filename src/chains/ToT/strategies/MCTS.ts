...
Generate ${this.ToT.config.thoughtsCount || 3} thoughts that continue this path.
    ...
    public async logic(tot: TreeOfThoughts): Promise<LogicReturnType> {
        ...
        for (let i = 0; i < this.config.iterations; i++) {
            this.telemetry?.recordIteration(i);
            // Selection
            this.telemetry?.recordStep("selection", { iteration: i });
            let path = [rootId];
            ...
            // Expansion
            this.telemetry?.recordStep("expansion", { currentId, depth });
            ...
            if (depth < (tot.config.maxThoughtsDepth ?? 10)) {
                ...
                if (newThoughts.length > 0) {
                    ...
                    this.telemetry?.recordStep("terminal_leaf", { currentId, reason: "no_thoughts" });
                ...
                // Max depth
                this.telemetry?.recordStep("terminal_leaf", { currentId, reason: "max_depth" });
                const score = currentNode.type === "option-node" ? (currentNode.initialRate?.score || 0) : (currentNode.rate?.score || 0);
                this.backpropagate(path, score, depth);
            }
        }
