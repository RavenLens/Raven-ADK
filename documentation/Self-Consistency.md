...
Each `invoke` call, including calls made through `check`, creates a
`self_consistency.invoke` span when OpenTelemetry is configured. The span
records the configured candidate count, agreement threshold, and minimum
candidate threshold when it starts. It records the final status, valid and
invalid candidate counts, cluster count, agreement, winner, and abstention
reason when applicable.

The span also emits `self_consistency.candidate` for each valid candidate,
`self_consistency.invalid_candidate` for each candidate excluded during
extraction, normalization, or weighting, and
`self_consistency.decision` after the final policy decision. Candidate runner
spans remain nested beneath this decision span when the runners provide their
own telemetry.
